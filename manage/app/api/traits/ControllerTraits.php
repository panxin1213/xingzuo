...
trait ControllerTraits
{
    ...
    {
        if (empty(Request()->header("Origin")) && empty(Request()->header("showjson")) && !empty($data) && empty($this->request->param('showjson'))) {
            echo '<pre>';
            print_r($data);
            echo '</pre>';
            return '';
    ...
    protected function canReturnParam($param)
    {
        $inparam = $this->request->param();
        $fields = ['timetype', 'fromtime', 'totime'];
        $param = array_filter($param, function ($v) use ($fields) {
            return in_array($v, $fields);
        }, ARRAY_FILTER_USE_KEY);
        $inparam = array_filter($inparam, function ($v) use ($fields) {
            return in_array($v, $fields);
        }, ARRAY_FILTER_USE_KEY);
        $data = array_merge($inparam, $param);
        isset($data['timetype']) && $data['timetype'] = intval($data['timetype']);
        return $data ?: null;
    }
}
